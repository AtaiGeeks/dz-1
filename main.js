...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- Рекурсивное движение маленького блока ---
  const smallBlock = document.getElementById('small-block');
  const bigBlock = document.getElementById('big-block');
  const moveBtn = document.getElementById('move-btn');

  if (smallBlock && bigBlock && moveBtn) {
    let pos = 0;
    const maxPos = bigBlock.clientWidth - smallBlock.clientWidth;

    function moveRight() {
      if (pos < maxPos) {
        pos += 1; // скорость движения (пиксели)
        smallBlock.style.left = pos + 'px';
